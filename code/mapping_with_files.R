## Functions written by Simon Perkins (slurmclassic)
# https://github.com/CBFLivUni/mapping_and_enrichment

# Change the two following entries to point to paths on your system for filtered mapping files.
idmapping_file_path = './data/mappings_9796_20220728132616.txt'
idmapping_selected_file_path = './data/mappings_selected_9796_20220728132616.txt'
...
map_using_uniprot_files <- function(input_id = 'UniProtKB-AC', input_values, output_id) {
  ...
  if (output_id %in% uniprot_selected_term_types) {
    ...
    data_terms = tibble(input_ids = input_values) %>% left_join(species_mapping_data, by = c('input_ids' = input_id)) %>% dplyr::select(input_ids, output_id)
    ...
    if (input_id != 'UniProtKB-AC') {
      input_values = species_mapping_data %>% filter(id_type == input_id) %>% filter(sapply(id, function(id_){id_ %in% input_values})) %>% dplyr::select(accession) %>% pull()
    }

    data_terms = tibble(input_ids = input_values) %>% left_join(filter(species_mapping_data, id_type == output_id), by = c('input_ids' = 'accession')) %>% dplyr::select(-id_type) %>% rename_with(\(x) output_id, matches('^id$'))
  }
