...
For GO term analysis we needed to manually annotate the proteomics and sequencing data. The GO term annotation for proteomics data can be readily accessed using the Uniprot accessions. 
...
gseaplot2(gsea_plasma_f2, geneSetID = 1, title = gsea_plasma_f2$Description[1])
...
Only factor 5 and factor 3 appear to have any significant GO terms associated with them. This is expected considering their association with the phenotype in the factor analysis. 
...
gsea_sf_f5 <- GSEA(geneList = factor5,
               exponent = 1,
               minGSSize = 5,
               maxGSSize = 500,
               pvalueCutoff = 0.05,
...
gseaplot2(gsea_sf_f5, geneSetID = 3, title = gsea_sf_f5$Description[3])
...
Factor 3 seems to be associated with an immune response. 

```{r}

gseaplot2(gsea_sf_f3, geneSetID = 1, title = gsea_sf_f3$Description[1])
...
## Cluster GSEA
ggsave(
  filename = "synovial_fluid_f5_gsea.PDF",
  plot = sf_f5_dp,
  width = 5.5,
  height = 4,
  units = c("in"),
